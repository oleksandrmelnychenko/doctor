---
abstract: |
  Authenticated channels can deliver media that remain ambiguous or unsuitable for local use. We present a transport-independent, policy-indexed method that reconciles filename, declared type, and bounded byte evidence; selects one source profile; enforces the required structural or semantic reconstruction; validates the complete candidate on a code path separate from construction; and publishes only clean, non-overwriting output. Conditional canonical-delivery and fixed-witness monotonicity results make authorizing-parser, construction, and filesystem assumptions explicit. The Rust implementation covers image, audio, ISO base media, and WebM profiles. In a bounded experiment, fixture-derived actions matched for 104 generated benign or inert-unsupported records, measuring fixture--policy consistency rather than independent ground truth. A later versioned rehearsal completed differential, fidelity, paired-ablation, 14-case fault, and available-regeneration checks, but used an under-budget generated corpus. No public source received independent admission, and the prescribed fuzzing and controlled-performance schedules were not run on a dedicated host. Consequently, $G_{\mathrm{sci}}$ and release eligibility remain false: the retained observations support the method and finite-run traceability, not population efficacy, malware detection, capacity, or release.
...
Digital media cross local trust boundaries through email, web download, cloud synchronization, direct import, and messaging. Upstream controls can establish channel authenticity, access rights, or confidentiality, but they do not prove that the received octet string is admissible to a local parser or decoder. It may still preserve privacy-bearing metadata, amplify resource demand, exploit a decoder, or admit conflicting parser interpretations. End-to-end encrypted messaging is one instance because a blind relay cannot inspect plaintext without changing the confidentiality model [1–3]. The underlying file-admission problem is not specific to that transport.

A suffix and caller-supplied media type are declarations, a magic-byte match may cover only a prefix, and a signature scan cannot establish whole-object grammar or removal of excluded structures. A successful transcoder exit proves only that one tool emitted bytes. Each mechanism supplies evidence, but none alone authorizes the emitted object for an endpoint filesystem or renderer.

Content disarm and reconstruction methods address part of this problem by replacing input with a reduced representation. Prior work establishes the importance of parser agreement, complete consumption, generated parsers, and canonical languages [4–10]. The unresolved systems question addressed here is how route selection, reconstruction strength, complete-output authorization, and filesystem publication compose at one local trust boundary.

This study develops a transport-agnostic method for policy-indexed canonical media reconstruction and fail-closed delivery. *Delivery* is any exposure of authorized bytes through the component interface; *publication* is the narrower non-overwriting filesystem transition. The method converges portable name, supplied type, and bounded byte evidence to one source profile, enforces the policy-required reconstruction level, reparses the complete candidate on a code path separate from construction, applies rebuilt-output scanning and resource bounds, and permits publication only after a clean verdict. Every other outcome exposes no deliverable bytes and leaves any pre-existing target unchanged.

The analytical claim is conditional: under registered-parser, semantic-construction, and filesystem assumptions, accepted output belongs to one policy-indexed language and a non-deliverable result cannot publish. Empirical conformance is falsified when a manifest record violates any prespecified action, reason, state, reconstruction, closure, resource, or fault oracle. Fidelity remains a separate usability condition and cannot authorize an invalid structure.

The novelty is the authorization relation: successful transformation is not authorization, reconstruction strength is policy checked, and publication is a separate guarded transition. Four linked contributions follow. *C1 (method)* composes unique routing, a registered source--level--output mapping, reconstruction sufficiency, complete candidate validation, rebuilt-output scanning, resource bounds, and clean-only delivery. *C2 (analysis)* states the assumption-to-claim boundary for conditional canonical delivery and fixed-witness post-reconstruction monotonicity. *C3 (system)* realizes the eight auditable obligations F1--F8 with typed failures, validation separate from construction, and non-overwriting publication. *C4 (evaluation protocol)* binds each obligation to record-level falsifiers, retains negative and incomplete runs, and withholds controlled or release claims whenever a scientific gate is not passed. C4 is a protocol contribution, not a completed population-level empirical result.
...
The scope is measurable carrier structure. Malware family and operational role require behavioral evidence unavailable from media bytes alone. The method does not remove harmful decoded meaning, prove absence of steganography, compensate for a compromised endpoint, or make an unsafe decoder trustworthy. For frozen profiles and explicit assumptions, it restricts publication to candidates validated separately from construction and produced at or above the required reconstruction level.
...
Throughout, let $\mathbb O=\{0,\ldots,255\}$ be the octet alphabet and $\mathbb O^*$ the set of finite byte strings. For $b\in\mathbb O^*$, $|b|$ denotes octet length; $[P]\in\{0,1\}$ is the Iverson indicator of proposition $P$; and $\bot$ denotes an undefined selector result or an absent published path, according to type. Sets and finite maps are compared only within their declared domains; all indexed sums, products, and conjunctions use the finite index sets stated locally. One invocation fixes policy snapshot $p$ and parser-registry version $v$. Subscripts retain them only where policy comparison or evidence traceability would otherwise be ambiguous.
...
| Decision | $\ensuremath{\mathsf{Accept}}_{p,v},\mathsf{Pub}_{p,v}$ | Candidate authorization and non-overwriting publication transition |
...
| --- | --- | --- |
| Signature and malware scanning [13] | Supporting evidence for F8 | No direct F1--F7 guarantee; a clean scan does not authorize grammar or reconstruction |
| Canonical/generated parsing [4, 7–9] | F1--F2 | Does not jointly enforce F3--F8 at reconstruction and publication boundaries |
| Content disarm and reconstruction [10, 16, 19–20] | Format-dependent F2/F4 evidence | Transformation success alone does not establish F1, F3, F5--F8 |
| Parser differentials and polyglot analysis [5, 17–18] | Evidence for F1/F3 | Detects disagreement but does not supply reconstruction, bounds, or clean-only publication |
...
: **Table 2.** Direct overlap between representative prior-work lines and obligations F1--F8. "Evidence" means that a line can test or motivate an obligation but does not establish it as a publication-boundary guarantee. {#tab:research-position}

[Table 2](#tab:research-position) does not claim that the cited systems are weaker within their own scopes. It identifies the combination studied here: unique routing, explicit reconstruction strength, complete validation of emitted bytes, resource control, and publication restricted to clean verdicts, with a distinct falsifier for every obligation.
...
**Theorem 5** (Fixed-witness post-reconstruction validation monotonicity). *For a fixed post-reconstruction witness $w$ and parser-registry version $v$, $$
...
**Theorem 9** (Conditional canonical delivery under registered-parser soundness). *Under [\[asm:parser-soundness,asm:semantic-construction,asm:publication\]](#asm:parser-soundness,asm:semantic-construction,asm:publication){reference-type="ref+label" reference="asm:parser-soundness,asm:semantic-construction,asm:publication"}, $$
\begin{aligned}
...
[Theorem 4.9](#thm:conditional-delivery) is deliberately shallow given its assumptions: the acceptance predicate is engineered so that soundness of the hand-written authorizing parsers is the sole load-bearing premise of canonical membership. [Assumption 4.6](#asm:parser-soundness) is therefore the central limitation of the analytical contribution, and no output is claimed canonical independently of that premise. It is not discharged here; the differential and fuzzing protocol of [Section 10](#sec:evaluation) can expose, but cannot prove the absence of, an unsound parser. A generated or mechanically verified authorizing parser for at least one central output profile remains future work. [Theorem 4.9](#thm:conditional-delivery) permits coincidental byte equality and does not exclude harmful decoded meaning, and none of its clauses proves codec correctness or malware absence.
...
**Deployment security contract.** Four claims remain distinct. Process supervision bounds the direct child's lifetime, diagnostics, and candidate size during ordinary failure or hang behavior; on Linux, `SIGXFSZ` is classified as an exceeded output bound. Decoder isolation is not supplied: protocol restrictions constrain library input selection but not arbitrary system calls after code execution is compromised, so filesystem authority must be narrowed by an enclosing sandbox. Publication provides non-overwrite and only platform-conditional atomic visibility. Crash durability is outside the contract because the parent directory is not synchronized. Thus supervision, isolation, visibility, and power-loss durability must not be reported as one guarantee.
...
Every accepted output is checked by the authorizing parser and at least two applicable external format or profile validators. Distinct families use different executable or library lineages. Separate process boundaries alone do not establish diversity. Reports bind output digests to executable versions and hashes, material parser or codec dependencies, detected profiles, and outcomes; validators are grouped by implementation lineage before the diversity count is evaluated. Any missing, rejecting, or disagreeing result fails the gate. These external checks corroborate decodability or profile recognition only. They do not independently establish complete consumption, component closure, the expected action, or the fixture-derived decision oracle, and they cannot exclude common-mode errors.
...
[Table 14](#tab:review-disclosures) makes the executed fault schedule and metric change auditable at manuscript scale. Panel A is bound to `aura-fault-injection-v4` at source commit `50e0c8ff7a7422c2aa5b55ad2b7ad80d6720f84e`; full test names and command-output digests remain in the linked evidence. Panel B compares the 80 sample identities retained by the original and readiness fidelity reports. Sixty-four output digests were identical. All 16 video digests changed, so their improved result cannot be attributed to the evaluator change alone.

| Panel / item | Injection or output identity | Required observation or original v3 result | Observed state or readiness v5 result | Delivered bytes / cleanup / frozen criterion |
| --- | --- | --- | --- | --- |
| **A. Fault schedule** |  |  |  |  |
| Pre-cancelled token | Before decoding | Stop before decoding | cancelled | 0; cleanup passed |
| Expired deadline | Before processing | Stop before processing | timed-out | 0; cleanup passed |
| Missing child executable | Launch boundary | No deliverable artifact | launch-failed | 0; cleanup passed |
| Child exceeds timeout | Process supervisor | Terminate child/process group | timed-out | 0; cleanup passed |
| Diagnostic flood | Diagnostic capture | Bound captured diagnostics | terminated | 0; cleanup passed |
| Oversized child output | Output monitor | Terminate before publication | terminated | 0; cleanup passed |
| Incomplete output parse | Authorizing parser | Prevent candidate delivery | validation-failed | 0; cleanup passed |
| Handler panic | Construction handler | Return typed error, no artifact | panic-contained | 0; cleanup passed |
| Blocking signature | Publication boundary | Deny non-clean publication | publication-denied | 0; cleanup passed |
| Allocator failure | Before publication | Leave no artifact or staging residue | publication-failed | 0; cleanup passed |
| Temporary-file failure | Before staging | Leave no artifact or staging residue | publication-failed | 0; cleanup passed |
| Persistence failure | After staging sync | Remove staging, publish nothing | publication-failed | 0; cleanup passed |
| Pre-existing destination | Destination check | Preserve bytes exactly | publication-denied | 0; cleanup passed; hash equal |
| Concurrent destination race | Before `persist_noclobber` | Preserve winner, remove losing stage | publication-failed | 0; cleanup passed; winner kept |
| **B. Fidelity versions** |  |  |  |  |
| Static image/GIF (32) | 32/32 output digests identical | 32/32 passed | 32/32 passed | Dimensions/canvas/frame agreement; PSNR $\ge40$ dB; timing drift $\le2\%$ where applicable |
| Audio (32) | 32/32 output digests identical | Container-duration comparison; maximum relative drift 0.129482; 0/32 passed | Decoded-sample duration; maximum drift 0; 32/32 passed | Equal channels, duration drift $\le2\%$, SI-SDR $\ge20$ dB |
| Video (16) | 0/16 output digests identical | Unnormalized source-WebM comparison; minimum SSIM 0.667352; 8/16 passed | Normalized timebase/frame-rate comparison; minimum SSIM 0.969366; 16/16 passed | Equal dimensions, duration drift $\le2\%$, SSIM $\ge0.90$ |

: **Table 14.** Review disclosures from the controlled-readiness evidence. Panel A lists every executed fault case. Panel B separates metric-version effects from changed output identity; thresholds were not relaxed. {#tab:review-disclosures}

The audio comparison isolates an evaluator change on byte-identical outputs. The video comparison does not: reconstruction and evaluator versions changed together. It is therefore a versioned before/after observation, not a paired estimate of a metric-only effect. Neither panel upgrades the under-budget, non-admitted rehearsal to controlled or population evidence.
...
Current evidence comprises implementation traceability, the release-ineligible 104-record server experiment, a 129-record S0--S7 readiness inventory, a ten-file public-benign compatibility pilot, and a 316-object candidate inventory. The readiness cycle corrected the audio metric on byte-identical outputs, recorded v5 video metrics on regenerated outputs, completed the paired-ablation and fault rehearsals, and regenerated six available outputs. The review packet verified 316 candidate identities and retained 948 inspection outputs, but unsigned selection returned 0/246 sources. These records make the remaining work executable; they do not establish an admitted corpus, efficacy, or release.

Next, the write-disabled handoff archive must be delivered outside the audit host to reviewers with distinct key lineages and independence clusters. Matching signed decisions must precede selection, S0/S1 assignment, full-file admission, and a protocol-budget S0--S7 manifest. The controlled run must then move to a single-purpose host, execute the 21-hour fuzz and 10-session performance schedules with 10,000 hierarchical bootstrap replicates, and regenerate the evidence package, including the fault and ablation schedules, against that admitted frozen corpus.

# Conclusion {#sec:conclusion}

Under the registered-parser, implementation-correctness, and filesystem assumptions, the proposed method governs a transport-independent local transition from untrusted media to a deliverable reconstructed object. Delivery requires one source route, a registered source-to-output mapping, policy-sufficient reconstruction, complete authorizing validation on a code path separate from construction, a clean result, and non-overwriting publication. Reconstruction success creates only a candidate. Every other result returns no deliverable bytes and preserves the non-overwrite postcondition.

The analytical model answers RQ1 and RQ2 conditionally. RQ3 remains incomplete. The bounded experiment matched fixture-derived oracles for 104 generated records; the later 129-record rehearsal completed differential, versioned fidelity, paired-ablation, 14-case fault, and available-regeneration checks. These observations did not become controlled evidence: no independent decision was signed, 0/246 public sources were admitted, the host was shared, and the prespecified fuzz and performance schedules were not run. Only schema and fault gates passed, so $G_{\mathrm{sci}}$ and release eligibility remained false. The retained handoff makes the missing admission and controlled run executable; scientific release remains pending.

# Data, Code, and Evidence Availability {#sec:data-availability .unnumbered}

Git-safe implementation and evidence records are retained in the private [Ecliptix File Defender repository](https://github.com/oleksandrmelnychenko/ecliptix-file-defender) and four immutable anchors: the [bounded server experiment](https://github.com/oleksandrmelnychenko/ecliptix-file-defender/tree/823a5ec7a0dd56a5ba1c12a5aa45c77a4bff71c0/docs/research/rehearsals/20260720T130631Z-server-experiment-dcd6e114d21a), the [public-source pilot prerelease](https://github.com/oleksandrmelnychenko/ecliptix-file-defender/releases/tag/evidence-public-source-pilot-20260724T215601Z-39bf19f1d17b-c44fdc45), the [offline reviewer-handoff prerelease](https://github.com/oleksandrmelnychenko/ecliptix-file-defender/releases/tag/evidence-public-source-review-handoff-20260725T231947Z-8aa55ef), and the [controlled-readiness record](https://github.com/oleksandrmelnychenko/ecliptix-file-defender/tree/a7d4e4c0425a0265d3438d6a2474acc72de0933d/docs/research/rehearsals/20260727T151107Z-50e0c8f-controlled-readiness). The linked records retain commit identities, manifests, checksums, per-record results, gate matrices, and attestations; the prereleases retain the large archives. They are release-ineligible and contain no independent admission signature. If private at publication, the archives require a persistent public or controlled-access deposit.
