...
\centering
\begin{tikzpicture}[
  phase/.style={draw,rounded corners=2pt,align=center,text width=23mm,
    minimum height=18mm,font=\scriptsize,inner sep=1.7pt},
  ...
  failbox/.style={draw,rounded corners=2pt,align=center,text width=47mm,
    minimum height=10mm,font=\footnotesize,inner sep=2.5pt,fill=gray!12},
  arrow/.style={-{Latex[length=2mm]},thick},
  fail/.style={-{Latex[length=2mm]},dashed,thick},
  failbranch/.style={dashed,thick}
]
\node[freeze] (p1) {\textbf{1. Freeze}\\[-1pt]
  run ID and\\input hashes};
\node[freeze,right=3mm of p1] (p2) {\textbf{2. Build}\\[-1pt]
  release build;\\tests, lint, audit};
\node[functional,right=3mm of p2] (p3) {\textbf{3. Functional}\\[-1pt]
  corpus and ablation;\\differential and fidelity};
\node[adversarial,right=3mm of p3] (p4) {\textbf{4. Adversarial}\\[-1pt]
  fuzz budgets and\\injected faults};
\node[adversarial,right=3mm of p4] (p5) {\textbf{5. Measurement}\\[-1pt]
  pinned\\performance runs};
\node[terminal,right=3mm of p5] (p6) {\textbf{6. Reduction}\\[-1pt]
  regenerate artifacts;\\evaluate $G_{\mathrm{sci}}$};
\draw[arrow] (p1) -- (p2);
\draw[arrow] (p2) -- (p3);
\draw[arrow] (p3) -- (p4);
\draw[arrow] (p4) -- (p5);
\draw[arrow] (p5) -- (p6);
\foreach \phase/\branch in {p1/f1,p2/f2,p3/f3,p4/f4,p5/f5,p6/f6}{
  \coordinate (\branch) at ($(\phase.south)+(0,-5mm)$);
  \draw[failbranch] (\phase.south) -- (\branch);
}
\draw[failbranch] (f1) --
  node[pos=0.76,above=0.5pt,font=\scriptsize,fill=white,inner sep=1pt]
  {failed or incomplete phase} (f6);
\coordinate (failmid) at ($(f3)!0.5!(f4)$);
\node[failbox,below=8mm of failmid] (failed) {retain the immutable run record;
  exclude downstream phases and release claims};
\draw[fail] (failmid) -- (failed.north);
\end{tikzpicture}
...
measurement and release-candidate eligibility. Dashed branches collect any
failed or incomplete phase without crossing the primary execution path.}
...
deadline. Let \(K_p^{\mathrm{out}}=K_{\mathrm{exact}}\mathbin{\dot\cup}
K_{\mathrm{lossy}}\). For \(k\in K_{\mathrm{lossy}}\), decoded
representation \(\phi_k\) maps into a registered metric space with finite
metric \(d_k\) and threshold \(\varepsilon_{p,k}\ge0\). Define
\begin{equation}
 \begin{gathered}
 \mathsf{Close}_{p,k}(y,y')=\\[-0.25ex]
 \begin{cases}
...
\end{equation}
For \(k=\mathsf{profile}(y)\), require
\[
\begin{aligned}
...
\(Y_i^{(c)}\in\{0,1,\bot\}\) is the observed decision for record \(i\), where
one denotes acceptance, zero denotes blocking, and \(\bot\) occurs exactly
when \(E_i^{(c)}=0\). The superscript zero denotes the eligible, successfully
replayed full-policy record. The paired transition counts and full-policy
denominators are
\begin{equation}
 \begin{aligned}
 N_{ab}^{(c)}
...
not establish diversity. Reports bind output digests to executable versions and
hashes, material parser or codec dependencies, detected profiles, and outcomes;
validators are grouped by implementation lineage before the diversity count is
evaluated. Any missing, rejecting, or disagreeing result
...
state, paths, child status, and cleanup. For every executed fault case
\(t\in\mathcal T_{\mathrm{fault}}\), let \(F_t^{\mathrm{pre}}\) denote failure
before publication and let \(B_t^{\mathrm{del}}\in\mathbb N_0\) be the number
of externally deliverable bytes. The required oracle is
\begin{equation}
...
clean deliverable \(i\) with \(|x_i|>0\) is
\begin{equation}
 e_i=\frac{|y_i|}{|x_i|},
 \label{eq:expansion}
\end{equation}
...
For a finite nonempty gate vector \(\mathbf g\in\mathbb G^J\), define
\begin{equation}
...
\end{equation}
Here \(\mathcal I\) is the frozen corpus, while \(\mathcal C\) remains the
ablation-condition set from \cref{subsec:mechanism-ablation}; provenance freezes
all input identities, schema validates cross-file consistency, \(g_r\) binds
each record-level oracle and evidence, and \(g_{F_j}\) instantiates
