...
\begin{longtable}{L{0.07\textwidth}L{0.19\textwidth}L{0.39\textwidth}L{0.24\textwidth}}
...
Second-pass stability operationalizes F5 but is not itself an acceptance
condition. Let \(\mathsf{Out}_{p,v}(u)\) be defined as \(y\) only when the
pipeline returns a clean accepted candidate
\(a=(y,n_o,m_o,k_o,\ell,e)\). Let
\(u[a]=((y,n_o,m_o),(p,d'))\) be a second request under the same policy and a
fresh, equivalent deadline context \(d'\). For profiles registered as exact,
\begin{equation}
 \mathsf{Out}_{p,v}(u)=y
 \Longrightarrow
 \mathsf{Out}_{p,v}(u[a])=y.
 \label{eq:idempotence}
\end{equation}
For lossy profiles, registered semantic closure instead requires a clean second
verdict, the same output profile and reconstruction requirement, and a
prespecified fidelity comparator. It does not imply perceptual equivalence or
bounded accumulated drift. The policy comparison in
\cref{eq:validation-monotonicity} holds the candidate and parser reports fixed.
End-to-end comparisons remain descriptive when route selection or
...
Controlled-run and release-candidate eligibility require the conjunction
\begin{equation}
 \begin{aligned}
 G_{\mathrm{sci}}={}&G_{\mathrm{prov}} \land G_{\mathrm{schema}}
 \land \bigwedge_{r\in\mathrm{corpus}}G_r
 \land \bigwedge_{j=1}^{8}G_{\mathrm{F}j} \\
 &{}\land G_{\mathrm{abl}} \land G_{\mathrm{diff}}
 \land G_{\mathrm{fidelity}}
 \land G_{\mathrm{fuzz}} \land G_{\mathrm{fault}}
 \land G_{\mathrm{perf}} \land G_{\mathrm{regen}}.
 \end{aligned}
 \label{eq:scientific-gate}
\end{equation}
\(G_{\mathrm{prov}}\) requires frozen source, dependency, policy, corpus, tool,
and environment identities. \(G_{\mathrm{schema}}\) requires every report to
pass its versioned schema and cross-file consistency checks. Each \(G_r\)
requires the expected action, reason family, pipeline state, digest, profile,
and per-record evidence for manifest record \(r\). The predicates
\(G_{\mathrm{F}1},\ldots,G_{\mathrm{F}8}\) correspond to the eight rows of
\cref{tab:property-suite}. \(G_{\mathrm{diff}}\) and
\(G_{\mathrm{fidelity}}\) require complete external-validation and fidelity
records for every accepted evaluation. \(G_{\mathrm{fuzz}}\),
\(G_{\mathrm{fault}}\), and \(G_{\mathrm{perf}}\) require their full
prespecified budgets and cases. \(G_{\mathrm{regen}}\) requires every table and
summary to regenerate from the retained raw records. Together with the ablation
gate, these predicates define controlled-run eligibility. A failed conjunct
precludes controlled, population, capacity, or release-candidate claims but
does not suppress finite observations or negative results. Failed runs remain
append-only, and a release candidate additionally requires
\texttt{release\_eligibility.eligible=true}.
