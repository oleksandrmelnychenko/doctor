...
observations, not causal mechanism estimates. A controlled rerun must add
preregistered challenge records that cross the prescan and supplied-type
decision boundaries; otherwise zero transitions remain uninformative about
those mechanisms. The full protocol-budget manifest
...
different reasons. The versioned comparison in
\cref{tab:review-disclosures} isolates the audio evaluator change because all
32 audio output digests were preserved. It does not isolate a video metric
effect because all 16 video outputs changed between runs; that improvement is
therefore descriptive and jointly confounded by reconstruction and evaluator
versions.
...
inventory. The readiness cycle corrected the audio metric on byte-identical
outputs, recorded v5 video metrics on regenerated outputs, completed the
paired-ablation and fault rehearsals, and regenerated six available outputs.
The review packet verified 316 candidate identities and retained 948 inspection
outputs, but unsigned selection returned 0/246 sources. These records make the
remaining work executable; they do not establish an admitted corpus, efficacy,
or release.
...
regenerate the evidence package, including the fault and ablation schedules,
against that admitted frozen corpus.
