...
Recent studies of file ambiguity and polyglots further demonstrate that extension,
media type, and isolated parser success do not determine one whole-file meaning
\cite{albertini2022,belkind2023,koch2025,you2025}.
...
For policy $p$ and profile $k$, let $L_{p,k}\subseteq\{0,1\}^{*}$ denote the
...
\begin{equation}
  V=\{\Clean,\Suspicious,\Blocked\},\qquad
  O=\mathsf{Result}(V\times E,\mathsf{PipelineError}),
  \label{eq:verdict-space}
\end{equation}
where $E$ contains bounded stage evidence. In enforcement mode, a clean verdict
...
Content disarm and reconstruction \cite{darpaSafeDocs,belkind2023,dubin2023} & Format-specific transformation into reduced representations & Policy-indexed claim strength and authorization of the emitted object rather than handler success \\
