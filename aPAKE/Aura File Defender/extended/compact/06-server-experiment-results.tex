...
Runner commit \texttt{\AFDReadinessRunnerCommit} retained
...
checks. Evidence commit: \texttt{\AFDReadinessEvidenceCommit} &
...
The readiness cycle preserved the original fidelity result as a separate
historical observation. In the versioned rerun, audio duration came from
decoded sample counts, which excluded container padding. Video comparison
normalized the source-WebM timebase and frame rate. No frozen threshold
changed. The maximum audio duration drift was \AFDReadinessAudioMaxDrift{}, and
the minimum video SSIM was \AFDReadinessVideoMinSsim{} against the unchanged
0.90 threshold. All \AFDReadinessFidelity{} fidelity records and all
\AFDReadinessDifferential{} differential records passed.

The generated readiness inventory contained \AFDReadinessRehearsalTotal{}
records with S0--S7 counts
...
were a scaffold, not independently adjudicated ground truth or a
protocol-budget corpus. In the paired baseline rehearsal, removing the input
signature prescan or supplied-type evidence caused no decision transition.
Lowering the reconstruction floor changed \AFDReadinessLoweredFloorTransitions{}
applicable full-policy accepts to blocks. These frozen-corpus transitions are
descriptive and do not identify a causal or population-level mechanism effect.
...
blocked pending independently signed admission and the protocol-budget run. The
archive contained \AFDReadinessArchiveBytes{} bytes with SHA-256 prefix
\texttt{\AFDReadinessArchiveHashShort}. No full fuzz hour, controlled
performance session, or bootstrap replicate was executed in this cycle.
